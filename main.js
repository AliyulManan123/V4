...
setInterval(async () => {
	...
	console.log(chalk.cyan(`Auto clear  | Folder tmp berhasil dibersihkan!`))
}, 180000) //3 muntos
...
global.reloadHandler = async function (restatConn) {
  ...
  conn.spromote = 'Selamat kak @user kamu sekarang jadi admin!'
  conn.sdemote = 'Yaah @user nggak jadi admin lagi!'
  conn.sDesc = 'Deskripsi dirubah \n@desc'
  conn.sSubject = 'Nama grup dirubah \n@group'
  conn.sIcon = 'Icon grup dirubah'
  conn.sRevoke = 'Link grup direset \n@revoke'
...
global.reload = async (_ev, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (err) conn.logger.error(`syntax error while loading '${filename}'\n${format(err)}`)
      ...
      conn.logger.error(`Error, Memerlukan plugin '${filename}\n${format(e)}'`)
    } finally {
