let handler = async (m, { conn, command, text, usedPrefix }) => {
  if (!text) throw `🚩 *Contoh:* ${usedPrefix + command} Shan|Bot`
  ...
  try {
    ...
    let [a, b] = text.split`|`     
    if (command == 'pornhub') {
      const res = `https://api.betabotz.org/api/textpro/pornhub?text=${encodeURIComponent(a)}&text2=${encodeURIComponent(b)}&apikey=${lann}`;
      await conn.sendFile(m.chat, res, 'textpro.jpeg', '', m);
    }
